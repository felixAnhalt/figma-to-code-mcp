...
describe.skipIf(process.env.RUN_FIGMA_INTEGRATION !== "1")(
  ...
  () => {
    ...
    it("compares raw Figma API response vs optimized MCP response (v2 CSS-aligned)", async () => {
      ...
      const componentCount = mcpResponse.components
        ? Object.keys(mcpResponse.components).length
        : 0;
      ...
      const hasBoundingBox = Object.values(mcpResponse.nodes).some(
        (n: any) => n.absoluteBoundingBox,
      );
